...
export default function DepartmentOperations({
  ...
  const [directIssueKg, setDirectIssueKg] = useState<number>(0);
  const [directIssueRemarks, setDirectIssueRemarks] = useState<string>('');
  ...
  const filteredItems = useMemo(() => {
    ...
    const isFilterActive = companyConfig?.customerItemFilterEnabled !== false;

    return savedItems.filter(item => {
      const itemParty = (item.partyName || item.customerName || '').trim().toLowerCase();
      // When customer filtering is active and customer is typed, ONLY show items that belong to that customer
      const matchesParty = (isFilterActive && activeParty) ? (itemParty === activeParty) : true;
  ...
  }, [savedItems, partyName, itemName, companyConfig?.customerItemFilterEnabled]);
  ...
  const filteredPurchaseItems = useMemo(() => {
    ...
    return savedItems.filter(item => {
      const itemParty = (item.partyName || item.customerName || '').trim().toLowerCase();
      const matchesSupplier = (isFilterActive && activeSupplier) ? (itemParty === activeSupplier) : true;
      const matchesQuery = !query || 
        item.itemName.toLowerCase().includes(query) || 
        (item.itemCode && item.itemCode.toLowerCase().includes(query));
  ...
  }, [savedItems, purchaseSupplier, purchaseItemName, companyConfig?.customerItemFilterEnabled]);
              ...
              <div>
                ...
                <div className="mt-1.5">
                  {companyConfig?.customerItemFilterEnabled === false ? (
                    <span className="text-[10.5px] text-slate-500 dark:text-slate-400 font-medium bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-700 block">
                      ℹ️ Customer item filtering disabled by Admin. Showing all {filteredItems.length} items.
                    </span>
                  ) : partyName.trim() ? (
                        ...
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3 text-xs">
                          <div onClick={() => onSelectJobCard(job)} className="cursor-pointer hover:underline min-w-0 flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              {hasDirectIssues && (
                                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900 text-emerald-700 dark:text-emerald-400 font-mono text-[9.5px] font-bold">
                                  <span>🏬 Store Issued:</span>
                                  {totalIssuedBag > 0 && <span>{totalIssuedBag} BAG</span>}
                                  {totalIssuedPcs > 0 && <span>{totalIssuedPcs} PCS</span>}
                                  {totalIssuedKg > 0 && <span>{totalIssuedKg} KG</span>}
                                </span>
                              )}
                            </div>
            ...
            <form onSubmit={handleDirectPurchaseEntry} onKeyDown={onTallyFormKeyDown} className="space-y-4 text-xs font-sans">
              ...
              <div>
                ...
                <div className="mt-1.5">
                  {companyConfig?.customerItemFilterEnabled === false ? (
                    <span className="text-[10.5px] text-slate-500 dark:text-slate-400 font-medium bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-700 block">
                      ℹ️ Customer item filtering disabled by Admin. Showing all {filteredPurchaseItems.length} items.
                    </span>
                      ...
                      <div className="space-y-3">
                        {storedItems.map(job => {
                          ...
                          return (
                            <SwipeableCard
                              key={job.jobCardNo}
                              onSwipeRight={onQuickTransfer ? () => onQuickTransfer(job) : undefined}
                              onSwipeLeft={() => {
                  ...
                  <AnimatePresence mode="popLayout">
                    {incomingTransfers.map(mov => {
                      ...
                      return (
                        <SwipeableCard
                          key={mov.movementId}
                          onSwipeRight={() => handleLocalAccept(mov)}
                          onSwipeLeft={() => {
                            setActiveRejectionId(isRejecting ? null : mov.movementId);
                            setRejectionNotes('');
                            setRejectQty(Number(mov.quantity) || 0);
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
                  <h3 className="font-sans font-bold text-sm text-slate-800 dark:text-white uppercase tracking-wider mb-3 flex items-center gap-2">
                    ...
                    <span className="text-xs font-normal font-mono text-slate-400">({dispatchStoreIssues.length})</span>
                  </h3>
                  {dispatchStoreIssues.length === 0 ? (
                    <div className="text-center py-6 space-y-2 border border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
                      <span className="text-xl">📦</span>
                      <p className="text-slate-400 text-xs font-mono font-medium">No Direct Store → Dispatch issues recorded yet</p>
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
